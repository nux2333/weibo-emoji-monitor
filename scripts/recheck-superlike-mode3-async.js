...
async function runRound(round) {
  ...
  for (const monitor of monitors) {
    const config = parseMonitorConfig(monitor.url); const users = await getUsers(monitor.id); stats.users += users.length; console.log(`[模式3] Monitor=${monitor.name} | UID=${users.length} | 条件=inserted_at今天+jyz>70 | 顺序=jyz DESC`); if (!users.length) continue; console.log('[模式3][队列TOP] ' + users.slice(0, 10).map(x => `${x.uid}(jyz=${x.experience_7d ?? '-'})`).join(' | '));
...
async function main() {
  if (!process.env.DATABASE_URL) throw new Error('缺少 DATABASE_URL'); createBatchLogger('recheck-superlike', 'mode3'); console.log('[模式3] 统一恢复策略：健康代理池轮询 + Cookie/IP绑定Session；4xx/5xx、网络错误、Session/Context/Browser关闭、Playwright timeout、visitor 自动重建Session'); console.log(`[模式3] 查询范围=inserted_at今天 + experience_7d>70 | PG pool max=${pool.options.max} | HTTP并发=${HTTP_CONCURRENCY} | UID上限=${BATCH_SIZE} | 单次建Session最多轮询代理=${SESSION_PROXY_RETRIES} | round=${ROUND_INTERVAL_MS / 1000}s`); let round = 0;
