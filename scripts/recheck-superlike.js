  ...
  return db.prepare(`
    ...
    ORDER BY
      ...
      CASE
        ...
        ELSE 1
      END ASC,
      datetime(comment_last_checked_at) ASC,
      id DESC
  `).all(
...
async function runLightSuperLikeRecheck(signal = null) {
  ...
  try {
    ...
    for (
      ...
      console.log('');
      console.log(
        `[轻量Profile] Monitor=${monitor.name} | UID=${users.length} | 条件=jyz>=70 | 顺序=jyz DESC`
      );
