...
app.get('/api/health', auth, (req, res) => {
  res.json({
    success: true,
    data: { host: os.hostname(), now: new Date().toISOString() }
  });
});
...
<div class="wrap">
  <div class="card">
    ...
    <div class="row" style="margin-top:12px">
      ...
      <input id="worker" class="top-input" placeholder="Worker，例如 PC-A">
      <button id="connect" class="btn blue" style="width:auto;padding:10px 16px">连接</button>
    </div>
    <div id="health" class="muted status">未连接</div>
  </div>

  <div class="card">
    <div class="muted">账号</div>
    <select id="account" class="account-select"><option value="">请先连接</option></select>
  </div>
  ...
  <div id="taskCard" class="card hidden">
    ...
    <a id="link" class="link" target="_blank" rel="noopener">查看帖子</a>
    <div class="gap"></div>
    <input id="manualText" class="input" autocomplete="off" placeholder="手动记录/评论内容；按 Enter = 确认完成并下一条">
    <div class="hint">Enter 只代表“我已经手动完成这条”，不会从这个网页向微博自动提交评论。</div>
    <div class="gap"></div>
    <button id="done" class="btn blue">完成并下一条</button>
    <div class="gap"></div>
    <button id="skip" class="btn secondary">跳过并下一条</button>
    <div id="status" class="status muted"></div>
  </div>

  <div id="empty" class="card hidden">
    <div class="title">这个账号没有待处理任务了</div>
...
(function(){
  ...
  async function api(url,opt){
    opt=opt||{};
    var r=await fetch(url,Object.assign({},opt,{headers:Object.assign({'Content-Type':'application/json','Authorization':'Bearer '+token.value.trim()},opt.headers||{})}));
    var j;try{j=await r.json()}catch(_){j={success:false,message:'HTTP '+r.status}}
    if(!r.ok||!j.success)throw new Error(j.message||('HTTP '+r.status));
    return j.data;
  }

  function setStatus(text,bad){var el=document.getElementById('status');el.textContent=text||'';el.className='status '+(bad?'bad':'muted')}

  function progressText(p){
    var total=Number(p.total_count||0),done=Number(p.done_count||0),skip=Number(p.skipped_count||0),pending=Number(p.pending_count||0);
    return {main:(done+skip)+' / '+total, detail:'完成 '+done+' ｜ 跳过 '+skip+' ｜ 剩余 '+pending};
  }

  async function loadAccounts(){
    var list=await api('/api/accounts?worker='+encodeURIComponent(worker.value.trim()));
    account.innerHTML='';
    if(!list.length){var o=document.createElement('option');o.value='';o.textContent='暂无已领取任务';account.appendChild(o);return}
    ...
    if(saved&&list.some(function(x){return x.account===saved}))account.value=saved;
    await loadNext();
  }
  ...
  async function loadNext(){
    ...
    if(!currentTask){
      taskCard.classList.add('hidden');empty.classList.remove('hidden');
      document.getElementById('emptyProgress').textContent=p.detail;
      return;
    }
    empty.classList.add('hidden');taskCard.classList.remove('hidden');
    document.getElementById('progress').textContent=p.main;
    document.getElementById('post').textContent=currentTask.post_text||'(无帖子文案)';
    document.getElementById('note').textContent=(currentTask.note||'')+'\n'+p.detail;
    var link=document.getElementById('link');link.href=currentTask.post_link||'#';link.style.display=currentTask.post_link?'inline-block':'none';
  ...
  async function finish(action){
    ...
    var done=document.getElementById('done'),skip=document.getElementById('skip');done.disabled=true;skip.disabled=true;
    try{
      await api('/api/task/'+encodeURIComponent(currentTask.task_id)+'/result',{method:'POST',body:JSON.stringify({worker:worker.value.trim(),account:account.value,action:action,manual_text:document.getElementById('manualText').value})});
      await loadNext();
    }catch(e){setStatus(e.message,true)}finally{done.disabled=false;skip.disabled=false}
  }

  async function connect(){
    sessionStorage.setItem('caToken',token.value.trim());localStorage.setItem('caWorker',worker.value.trim());
    try{var h=await api('/api/health');document.getElementById('health').innerHTML='<span class="ok">● 已连接</span> '+h.host;await loadAccounts()}catch(e){document.getElementById('health').innerHTML='<span class="bad">'+String(e.message)+'</span>'}
  }

  document.getElementById('connect').addEventListener('click',connect);
  account.addEventListener('change',loadNext);
  document.getElementById('done').addEventListener('click',function(){finish('done')});
  document.getElementById('skip').addEventListener('click',function(){finish('skip')});
  document.getElementById('manualText').addEventListener('keydown',function(e){if(e.key==='Enter'&&!e.shiftKey){e.preventDefault();finish('done')}});
  if(token.value&&worker.value)connect();
})();
</script>
</body>
</html>`;
