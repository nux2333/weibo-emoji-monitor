...
function currentTaskForAccount(worker, account) {
  return db.prepare(`SELECT a.task_id, t.post_id, t.post_link, t.post_text, t.note, a.claimed_at
    ...
    LIMIT 1`).get(worker, account) || null;
}
...
app.get('/api/my-tasks', userAuth, (req, res) => {
  ...
  const rows = db.prepare(`SELECT a.account, COUNT(*) AS assigned_count,
    ...
    WHERE a.worker_id = ? GROUP BY a.account ORDER BY MAX(a.claimed_at) DESC`).all(DEFAULT_TASK_ID, worker).map(row => {
      ...
      const current = Number(row.running_count || 0) > 0 ? currentTaskForAccount(worker, row.account) : null;
      return {
        ...row,
...
app.get('/api/my-tasks/:account/current', userAuth, (req, res) => {
  ...
  if (!current) return res.status(404).json({ success: false, message: '当前账号没有待处理任务' });
...
app.post('/api/my-tasks/:account/open-current', userAuth, async (req, res) => {
  try {
    ...
    touchWorker(worker, { account, status: 'waiting-manual', note: `${account}：已打开当前帖子，等待人工处理` });
    res.json({ success: true, data: { account, ...current } });
...
app.post('/api/my-tasks/:account/confirm-next', userAuth, (req, res) => {
  try {
    ...
    if (!account) return res.status(400).json({ success: false, message: 'account required' });
    if (confirmValue !== 'y') return res.status(400).json({ success: false, message: '请输入 y 确认当前任务' });

    const current = currentTaskForAccount(worker, account);
    if (!current) return res.status(404).json({ success: false, message: '当前账号没有等待确认的任务' });

    db.prepare(`UPDATE comment_assistant_task_assignments
      SET status = 'DONE', result = '用户输入 y 确认完成', completed_at = LOCALTIMESTAMP
      WHERE task_id = ? AND worker_id = ? AND account = ? AND status = 'CLAIMED'`).run(current.task_id, worker, account);
    db.prepare(`UPDATE comment_assistant_tasks
...
(function(){
  var tokenEl=byId('token');var workerId='${DEFAULT_WORKER_ID}';var taskLoops=1;var taskInterval=0;var confirmDrafts={};tokenEl.value=sessionStorage.getItem('caToken')||'';
  ...
  async function copyText(text){var value=String(text||'');if(!value)return false;try{await navigator.clipboard.writeText(value);return true}catch(_){var ta=document.createElement('textarea');ta.value=value;ta.style.position='fixed';ta.style.opacity='0';document.body.appendChild(ta);ta.select();var ok=false;try{ok=document.execCommand('copy')}catch(__){}document.body.removeChild(ta);return ok}}
  ...
  async function loadTasks(){var list=await api('/api/my-tasks');var body=byId('tasks');body.innerHTML='';if(!list.length){body.innerHTML='<tr><td colspan="5" class="muted">暂无任务</td></tr>';return}list.forEach(function(item){var tr=document.createElement('tr');var account=document.createElement('td');account.textContent=item.account||'-';var taskName=document.createElement('td');taskName.textContent=item.task_name||'-';var progress=document.createElement('td');var hasRunning=Number(item.running_count||0)>0;progress.textContent=(hasRunning?'待处理 · ':'')+String(item.progress_count||0)+'/'+String(item.target_count||20);var confirmTd=document.createElement('td');if(hasRunning){var wrap=document.createElement('div');wrap.className='confirm-wrap';var input=document.createElement('input');input.type='text';input.inputMode='text';input.autocomplete='off';input.maxLength=1;input.placeholder='y';input.className='confirm-input';input.dataset.account=item.account;input.value=confirmDrafts[item.account]||'';var confirmButton=document.createElement('button');confirmButton.type='button';confirmButton.className='confirm-next';confirmButton.dataset.account=item.account;confirmButton.textContent='确认';wrap.appendChild(input);wrap.appendChild(confirmButton);confirmTd.appendChild(wrap)}else{confirmTd.textContent='-'}var actions=document.createElement('td');if(hasRunning){var openButton=document.createElement('button');openButton.type='button';openButton.className='open-current';openButton.dataset.account=item.account;openButton.textContent='打开帖子';actions.appendChild(openButton);actions.appendChild(taskActionButton(item.account,'interrupt','中断','gray'))}else{actions.appendChild(taskActionButton(item.account,'delete','删除','red'))}tr.appendChild(account);tr.appendChild(taskName);tr.appendChild(progress);tr.appendChild(confirmTd);tr.appendChild(actions);body.appendChild(tr)})}
  async function confirmNext(account,input,button){var value=String((input&&input.value)||'').trim().toLowerCase();if(value!=='y'){alert('请输入 y 确认当前任务');if(input)input.focus();return}if(button)button.disabled=true;if(input)input.disabled=true;try{var data=await api('/api/my-tasks/'+encodeURIComponent(account)+'/confirm-next',{method:'POST',body:JSON.stringify({worker:workerId,confirm:'y'})});confirmDrafts[account]='';log('账号 '+account+' → y确认完成1条 | 已完成='+data.done_count+' | 剩余='+data.remaining_count+' | Link='+(data.post_link||'-'));await loadTasks();var next=document.querySelector('.confirm-input[data-account="'+CSS.escape(account)+'"]');if(next)next.focus()}catch(err){alert(err.message);log('确认任务失败：'+err.message);if(input){input.disabled=false;input.focus()}if(button)button.disabled=false}}
  ...
  byId('tasks').addEventListener('input',function(e){if(e.target.classList.contains('confirm-input'))confirmDrafts[e.target.dataset.account]=e.target.value});
  byId('tasks').addEventListener('keydown',function(e){if(!e.target.classList.contains('confirm-input')||e.key!=='Enter')return;e.preventDefault();var account=e.target.dataset.account;var button=e.target.closest('td').querySelector('.confirm-next');confirmNext(account,e.target,button)});
  byId('tasks').addEventListener('click',async function(e){var openButton=e.target.closest('.open-current');if(openButton){var accountName=openButton.dataset.account;openButton.disabled=true;var oldOpen=openButton.textContent;openButton.textContent='打开中...';try{var current=await api('/api/my-tasks/'+encodeURIComponent(accountName)+'/open-current',{method:'POST',body:JSON.stringify({worker:workerId})});var copied=await copyText(current.post_text||'');log('账号 '+accountName+' → 已打开当前帖子 | Link='+(current.post_link||'-')+' | 评论文案'+(copied?'已复制':'复制失败'));if(!copied&&current.post_text)alert('帖子已打开，但浏览器未允许自动复制。请手动复制评论文案。')}catch(err){alert(err.message);log('打开当前帖子失败：'+err.message)}finally{openButton.disabled=false;openButton.textContent=oldOpen}return}var confirmButton=e.target.closest('.confirm-next');if(confirmButton){var confirmInput=confirmButton.closest('td').querySelector('.confirm-input');await confirmNext(confirmButton.dataset.account,confirmInput,confirmButton);return}var button=e.target.closest('.task-action');if(!button)return;var action=button.dataset.action;var account=button.dataset.account;if(action==='delete'&&!confirm('确定删除 '+account+' 的任务记录？未完成任务会释放回任务池。'))return;if(action==='interrupt'&&!confirm('确定中断 '+account+' 当前任务？'))return;button.disabled=true;try{await api('/api/my-tasks/'+encodeURIComponent(account)+'/action',{method:'POST',body:JSON.stringify({worker:workerId,action:action})});log('账号 '+account+' → '+action);await loadTasks()}catch(err){alert(err.message);log('任务操作失败：'+err.message)}finally{button.disabled=false}});
