...
function initializeBrowserProxy() {
  ...
  console.log('[浏览器代理] 登录、打开帖子、CSRF 与评论 POST 全程使用该代理，不在评论之间切换。');
  return BROWSER_PROXY;
}
...
async function readPostWithRotatingProxy(proxyPool, proxyIndex, url) {
  ...
  try {
    ...
    return { proxyIndex: (index + 1) % proxyPool.length, proxy, result: { status: response.status(), ok: response.ok() } };
  } catch (error) {
    return { proxyIndex: (index + 1) % proxyPool.length, proxy, result: { status: null, ok: false, error: error.message } };
  } finally {
...
async function waitForManualLoginConfirmation(context, existingRl = null) {
  const ownRl = existingRl || readline.createInterface({ input, output });
  try {
    while (true) {
      await ownRl.question('[登录] 请在 Chrome 中完成微博登录；确认已经登录后按 Enter 继续：');
      if (await hasWeiboLogin(context)) return;
      console.log('[登录] 还没有检测到登录 Cookie，Chrome 会继续保持打开，请完成登录后再按 Enter。');
    }
  } finally {
    if (!existingRl) ownRl.close();
  }
}
async function interactiveLogin(oldContext, clearCookies, reason, existingRl = null) {
  ...
  await waitForManualLoginConfirmation(context, existingRl);

  console.log('[登录] 已确认登录，保存登录信息并切回后台运行。');
  await context.close();
...
async function ensureLoggedIn() {
  ...
  return interactiveLogin(context, false, '[登录] 当前账号没有登录信息，正在打开 Chrome，请手动登录微博。');
}
async function getCsrfToken(page,context) {
  ...
  return page.evaluate(() => {
    for (const selector of ['meta[name="csrf-token"]','meta[name="csrf_token"]','meta[name="xsrf-token"]','meta[name="x-xsrf-token"]']) {
      const value = document.querySelector(selector)?.getAttribute('content');
      if (value) return { token:value, source:`meta:${selector}` };
    }
    const globals = [
    ...
    for (const [source,value] of globals) if (value) return { token:String(value), source };
    return null;
  }).catch(() => null);
}
async function sendComment(page,context,postId,commentText) {
  const csrf = await getCsrfToken(page,context);
  if (!csrf?.token) return { ok:false,status:0,json:null,text:'CSRF token not found',csrfSource:null };
  const result = await page.evaluate(async({postId,commentText,fp,csrfToken}) => {
    const form = new URLSearchParams();
    Object.entries({id:String(postId),comment:commentText,pic_id:'',is_repost:'0',comment_ori:'0',is_comment:'0'}).forEach(([k,v]) => form.set(k,v));
    if (fp) form.set('fp',fp);
    const response = await fetch('/ajax/comments/create', {
      method:'POST', credentials:'include',
      headers:{'Content-Type':'application/x-www-form-urlencoded; charset=UTF-8','X-Requested-With':'XMLHttpRequest','X-XSRF-TOKEN':csrfToken,'X-CSRF-TOKEN':csrfToken,Accept:'application/json, text/plain, */*'},
      body:form.toString()
    });
    const text = await response.text();
    let json = null; try { json = JSON.parse(text); } catch {}
    return { ok:response.ok,status:response.status,json,text };
  }, {postId,commentText,fp:COMMENT_FP,csrfToken:csrf.token});
  return {...result,csrfSource:csrf.source};
}
...
function summarizeResult(result) {
  ...
  return [`HTTP ${result.status}`,code!==null?`code=${code}`:'',message?`msg=${message}`:'',result.csrfSource?`csrf=${result.csrfSource}`:''].filter(Boolean).join(' | ');
}
async function main() {
  initDatabase();
    ...
    for(let i=0;i<targets.length;i+=1) {
      ...
      try {
        ...
        if(isLoginExpiredResult(result)) {
          ...
          context=await interactiveLogin(context,true,'[登录] 正在打开 Chrome，请重新登录微博。',rl);
