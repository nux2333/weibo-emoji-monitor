...
if (enabled) {
  ...
  function isQaUrl(url) {
    try {
      const parsed = new URL(String(url || ''));
      return parsed.searchParams.get('flowId') === QA_FLOW_ID;
    } catch {
      return String(url || '').includes(QA_FLOW_ID);
    }
  }
  ...
  proxyModule.SCAN_PROXY_POOL.acquire =
    async function acquireQaTrackedProxy(...args) {
      const assignment =
  ...
  async function ensureHttpOnlySession(page) {
    ...
    creatingSession = (async () => {
      ...
      if (!browserContext) {
        throw new Error(
          'QA HTTP-only 无法取得 BrowserContext'
        );
      }
       ...
       * 在关闭 Chromium 前把它替换为纯 Node sleep，避免已关闭 Page 报错。
       */
      try {
        page.waitForTimeout =
      ...
      try {
        ...
        console.warn(
          `[SuperLike][QA HTTP-only] Chromium 关闭失败：${error?.message || error}`
        );
      }
  ...
  async function httpGet(
    ...
    for (
      ...
      console.log(
        `[SuperLike][QA HTTP-only] attempt=${attempt}/${maxAttempts} | HTTP=${lastResult.httpStatus ?? '-'} | ${lastResult.elapsedMs}ms | ${url}`
      );
  ...
  chaohuaApi.fetchChaohuaInPage =
    async function fetchQaHttpOnly(
      page,
      url,
      headers = {}
    ) {
      /*
       * QA flow 出现之前仍沿用共通 Scan HTTP 层，
       * 让真实页面有机会完成所有初始化。
       */
      if (!isQaUrl(url)) {
  ...
  profileApi.checkUserSuperLikeByProfile =
    async function checkQaProfileByHttp(
      context,
      config,
      uid,
      ...
      if (!httpContext) {
        console.warn(
          `[SuperLike][QA HTTP-only][Profile] UID=${uid} HTTP session 尚未建立，临时回退原 Profile。`
        );
      ...
      console.log(
        `[SuperLike][QA HTTP-only][Profile] UID=${uid} | HTTP=${status} | SuperLike=${hasSuperLike} | Posts=${profilePosts.length}`
      );
  ...
  process.once(
    'exit',
    () => {
      /* APIRequestContext.dispose() 为 async，exit 阶段不等待；正常 scanner 结束由进程释放。 */
      httpContext = null;
    }
  );
